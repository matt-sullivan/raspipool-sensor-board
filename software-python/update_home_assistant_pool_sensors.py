...
def update_sensor_config(sensor_name, value_field, unit_of_measurement, device_class):
...
def update_sensor_values(sensor_name, values):
    mqtt_client = mqtt.Client()
    mqtt_client.connect(BROKER)
...
def update_all_sensor_configs():
    update_sensor_config("pool_ph", "ph", None, None)
    update_sensor_config("pool_orp", "orp", "mV", None)
    update_sensor_config("pool_temp", "temperature", "°C", "temperature")

def update_all_sensor_values():
    update_sensor_values("pool_ph",pool_sensors.read_ph())
    update_sensor_values("pool_orp", pool_sensors.read_orp())
    update_sensor_values("pool_temp", pool_sensors.read_temp())

if __name__ == "__main__":
    logging.basicConfig(level=logging.INFO)
    if len(argv) > 1 and argv[1] == 'config' :
        update_all_sensor_configs()
